...
class AirbusImage(object):
    ...
    def get_united_masks(self):
        ...
        united_mask = resize(united_mask, (512, 512), anti_aliasing=True)
        return united_mask
...
images_to_use, images_discarded = train_test_split(images_name, test_size=0.99)
...
def customLoss(yTrue, yPred):
    ...
    #print(yTrue)
    #print(yPred)
    # When expected 1 but predicted 0
    onesCondition = tf.equal(yTrue, 0.)
    diff = tf.where(onesCondition, diff * weights[1], diff)
    
    # When expected 0 but predicted 1
    zeroCondition = tf.not_equal(yTrue, 0.)
    diff = tf.where(zeroCondition, diff * weights[0], diff)
    #return K.mean(K.abs(diff))
    return K.mean(K.abs(diff))

sol = customLoss(tf.convert_to_tensor([0., 0., 1., 1.], dtype=tf.float32), tf.convert_to_tensor([0.,1.,0.,1.], dtype=tf.float32))
print(sol)
...
# Decoder
conv3 = Conv2D(128, (3, 3), activation='relu', padding='same')(conv2)#pool2)
conv3 = Dropout(0.2)(conv3)
conv3 = Conv2D(128, (3, 3), activation='relu', padding='same')(conv3)

#up1 = concatenate([UpSampling2D((2, 2))(conv3), conv2], axis=-1)
conv4 = Conv2D(64, (3, 3), activation='relu', padding='same')(conv3)#up1)
