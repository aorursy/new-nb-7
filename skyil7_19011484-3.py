...
l1 = torch.nn.Linear(783, 64, bias=True)
l2 = torch.nn.Linear(64, 64, bias=True)
l3 = torch.nn.Linear(64, 10, bias=True)
re = torch.nn.ReLU()
do = torch.nn.Dropout()
...
model = torch.nn.Sequential(l1, re, do, l2, re, do, l3).to(device)
...
submit.head()
submit.to_csv('my_sub.csv', index=0)
