...
scale_min  = x['scalar_coupling_constant'].min()

scale_max = x['scalar_coupling_constant'].max()

scale_mid = (scale_max + scale_min)/2

scale_norm = scale_max - scale_mid

noofmolecules = len(x.molecule_name.unique())

print(noofmolecules)
from tqdm import tqdm

train_nodes_array     = np.zeros((noofmolecules, 29, 29,2), dtype=np.float32) 

for i, (k, m) in enumerate(tqdm(x.groupby('molecule_name'))):

    ind = m[['atom_index_0', 'atom_index_1' ]].values

    out_edges = np.zeros((29, 29, 2))

    out_edges[ind[:,0], ind[:,1], 0] = (m['scalar_coupling_constant' ].values-84.3307)/120.5493

    out_edges[ind[:,1], ind[:,0], 0] = out_edges[ind[:,0], ind[:,1], 0]

    out_edges[ind[:,0], ind[:,1], 1] = (m['numtype' ].values)

    out_edges[ind[:,1], ind[:,0], 1] = out_edges[ind[:,0], ind[:,1], 1]

    train_nodes_array[i] = out_edges
outputwithclasses =  train_nodes_array.reshape(-1,29*29,2)
nodes_train, in_edges_train, out_labels = shuffle(nodes_train, in_edges_train, outputwithclasses)
...
class Message_Passer_NNM(tf.keras.layers.Layer):
    ...
    def __init__(self, node_dim):
        ...
        self.nn = tf.keras.layers.Dense(units=self.node_dim*self.node_dim, activation = None)#activation=tf.nn.relu)
...
def log_mae(orig , preds):
    ...
    return reconstruction_error
def GroupScore(orig , preds):

    # Mask values for which no scalar coupling exists

    cnt = tf.constant(1e-9)

    fullmask  = tf.keras.backend.flatten(orig[:,:,1])

    nums  = tf.math.multiply(tf.keras.backend.flatten(orig[:,:,0]), tf.constant(120.5493))

    preds = tf.math.multiply(tf.keras.backend.flatten(preds), tf.constant(120.5493))

    

    

    mask = tf.where(tf.keras.backend.equal(fullmask,tf.keras.backend.constant(1.)))

    a1 = (tf.reduce_mean(tf.keras.backend.mean(tf.keras.backend.abs(tf.keras.backend.gather(nums,mask)-

                                                                               tf.keras.backend.gather(preds,mask)))))

    

    

    mask = tf.where(tf.keras.backend.equal(fullmask,tf.keras.backend.constant(2.)))

    a2 = (tf.reduce_mean(tf.keras.backend.mean(tf.keras.backend.abs(tf.keras.backend.gather(nums,mask)-

                                                                               tf.keras.backend.gather(preds,mask)))))

    

    mask = tf.where(tf.keras.backend.equal(fullmask,tf.keras.backend.constant(3.)))

    a3 = (tf.reduce_mean(tf.keras.backend.mean(tf.keras.backend.abs(tf.keras.backend.gather(nums,mask)-

                                                                               tf.keras.backend.gather(preds,mask)))))

    

    mask = tf.where(tf.keras.backend.equal(fullmask,tf.keras.backend.constant(4.)))

    a4 = (tf.reduce_mean(tf.keras.backend.mean(tf.keras.backend.abs(tf.keras.backend.gather(nums,mask)-

                                                                               tf.keras.backend.gather(preds,mask)))))

    

    mask = tf.where(tf.keras.backend.equal(fullmask,tf.keras.backend.constant(5.)))

    a5 = (tf.reduce_mean(tf.keras.backend.mean(tf.keras.backend.abs(tf.keras.backend.gather(nums,mask)-

                                                                               tf.keras.backend.gather(preds,mask)))))

    

    mask = tf.where(tf.keras.backend.equal(fullmask,tf.keras.backend.constant(6.)))

    a6 = (tf.reduce_mean(tf.keras.backend.mean(tf.keras.backend.abs(tf.keras.backend.gather(nums,mask)-

                                                                               tf.keras.backend.gather(preds,mask)))))

    

    mask = tf.where(tf.keras.backend.equal(fullmask,tf.keras.backend.constant(7.)))

    a7 = (tf.reduce_mean(tf.keras.backend.mean(tf.keras.backend.abs(tf.keras.backend.gather(nums,mask)-

                                                                               tf.keras.backend.gather(preds,mask)))))

    

    mask = tf.where(tf.keras.backend.equal(fullmask,tf.keras.backend.constant(8.)))

    a8 = (tf.reduce_mean(tf.keras.backend.mean(tf.keras.backend.abs(tf.keras.backend.gather(nums,mask)-

                                                                               tf.keras.backend.gather(preds,mask)))))

    #tf.keras.backend.log(tf.keras.backend.maximum(cnt,))

    classscores = tf.stack([a1,a2,a3,a4,a5,a6,a7,a8])

    mask = tf.where(tf.keras.backend.not_equal(classscores,tf.keras.backend.constant(0.)))

    vals = tf.keras.backend.gather(classscores,mask)

    vals = tf.math.maximum(cnt,vals)

    vals = tf.math.log(vals)

    vals = tf.reduce_mean(tf.keras.backend.mean(vals))

    return vals-tf.math.log(cnt)
learning_rate = 0.001
...
mpnn.compile(opt, GroupScore, metrics = [GroupScore])
