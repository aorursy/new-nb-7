...
def Aggregate(teamcompactresults1,
    ...
    return agg_results
def GrabData():
    tourneyresults = pd.read_csv('../input/NCAATourneyCompactResults.csv')
    tourneyseeds = pd.read_csv('../input/NCAATourneySeeds.csv')
    regularseasoncompactresults = \
        pd.read_csv('../input/RegularSeasonCompactResults.csv')
    sample = pd.read_csv('../input/SampleSubmissionStage1.csv')
...
def GPIndividual1(data):
    predictions = (1.0*np.tanh((((((((((((data["team2Seed"]) + (data["team1Lmin"]))/2.0)) + (data["team2Seed"]))/2.0)) + (data["team2Seed"]))/2.0)) - (data["team1Seed"]))) +
                   1.0*np.tanh((((((((((data["team1Lmin"]) / 2.0)) + (data["team2Seed"]))/2.0)) * (((data["team2Seed"]) * (((data["team2Seed"]) - (data["team1Seed"]))))))) / 2.0)) +
                   0.943544*np.tanh((((((((np.where(data["team1wins"]>0, ((data["team1losses"]) / 2.0), ((data["team1Lmin"]) - (data["team1WAverage"])) )) + (data["team1wins"]))/2.0)) / 2.0)) / 2.0)))
    return Outputs(predictions)


def GPIndividual2(data):
    predictions = (1.0*np.tanh((((((data["team2Seed"]) + (((((((data["team1Lmin"]) + (data["team2Seed"]))/2.0)) + (data["team2Seed"]))/2.0)))/2.0)) - (data["team1Seed"]))) +
                   1.0*np.tanh(((((((data["team1Seed"]) * (data["team1Seed"]))) - (data["team1Seed"]))) * (((((((data["team2Seed"]) - (data["team1Seed"]))) / 2.0)) / 2.0)))) +
                   0.942762*np.tanh(((((((((data["team2Seed"]) / 2.0)) * (data["team1Seed"]))) * ((((data["team2wins"]) + (data["team1Seed"]))/2.0)))) / 2.0)))
    return Outputs(predictions)


def GPIndividual3(data):
    predictions = (1.0*np.tanh((((((data["team2Seed"]) + (((((((data["team2Seed"]) + (data["team1Lmin"]))/2.0)) + (data["team2Seed"]))/2.0)))/2.0)) - (data["team1Seed"]))) +
                   1.0*np.tanh(((((((((data["team2Seed"]) - (data["team1Seed"]))) * ((((data["team1losses"]) + (((data["team1Seed"]) / 2.0)))/2.0)))) * (data["team1Seed"]))) / 2.0)) +
                   1.0*np.tanh(((((((data["team1Seed"]) - (data["team2Seed"]))) * (((((((data["team2Seed"]) / 2.0)) / 2.0)) * (data["team1Seed"]))))) / 2.0)))
    return Outputs(predictions)
