...
import gc; gc.enable() # for memory management in Kaggle
...
unique_img_ids = unique_img_ids[unique_img_ids['file_size_kb'] > 50] # image files less than 50kb will be corrupt  
...
early = EarlyStopping(monitor="val_loss", mode="min", verbose=2,
                      ...
                      patience=10) # since kaggle time is limited, we are limiting the patience to 10
...
# Predicting masks for the test images
...
# Encoded Pixels of the test images
...
pred.head()
## Displaying masks of the predicted ships



fig, m_axs = plt.subplots(10, 2, figsize = (9, 50))

[c_ax.axis('off') for c_ax in m_axs.flatten()]



for (ax1, ax2), c_img_name in zip(m_axs, pred.ImageId.unique()[:10]):
    ...
    ax2.imshow(masks_as_color(pred.query('ImageId=="{}"'.format(c_img_name))['EncodedPixels']))

    ax2.set_title('Prediction')
pred_with_ship = pred.ImageId[pred.EncodedPixels.isnull()==False]

pred_with_ship = np.unique(pred_with_ship.values)

print('There are ' +str(len(pred_with_ship)) + ' image files with masks')