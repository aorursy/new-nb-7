...
tr_te=tr[tr.columns.difference(["deal_probability"])].append(te)\
     ...
     .pipe(Concat_Text,['title','description'],'txt2').pipe(Ratio_Words).pipe(Lenght_Columns,['title','description','param_1']).assign( category_name_en=lambda x: pd.Categorical(x['category_name_en']).codes,
              ...
              region_en=lambda x:pd.Categorical(x['region_en']).codes, 
...
m_lgb=lgb.train(params=Dparam,train_set=dtrain,num_boost_round=30000, early_stopping_rounds=50, valid_sets=[dtrain,dval], verbose_eval=50,valid_names=['train','valid'])
...
LGB_text.to_csv("Blurrness_nans_0.03_15000.csv", index=False)