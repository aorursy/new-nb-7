...
def load_and_prec():
    ...
    trainx = train_df["question_text"].fillna("_##_").values
    test_X = test_df["question_text"].fillna("_##_").values
    ## Get the target values
    trainy = train_df['target'].values
    
    ## Split the train data
    train_X, val_X, train_y, val_y = train_test_split(trainx, trainy, test_size=0.03, stratify=trainy, random_state=SEED)
    ...
    train_features, val_features ,a, b=  train_test_split(features, trainy, test_size=0.03, stratify=trainy, random_state=SEED)
...
#     x_test_f, features_t = zip(*x_test_f)    
    ...
    return train_X, val_X, test_X, train_y, val_y, train_features, val_features, test_features, tokenizer.word_index
...
x_train, x_val, x_test, y_train, y_val, train_features, val_features, test_features, word_index = load_and_prec() 
...
# matrix for the predictions on the val set
...
x_val_cuda = torch.tensor(x_val, dtype=torch.long).cuda()
val = torch.utils.data.TensorDataset(x_val_cuda)
...
#     model = BiLSTM(lstm_layer=2,hidden_dim=40,dropout=DROPOUT).cuda()
    ...
    # predict all samples in the val set batch per batch
    for i, (x_batch,) in enumerate(val_loader):
        f = val_features[i * batch_size:(i+1) * batch_size]
        y_pred = model([x_batch,f]).detach()

        val_preds_fold[i * batch_size:(i+1) * batch_size] = sigmoid(y_pred.cpu().numpy())[:, 0]

    val_preds += val_preds_fold / len(splits)
...
def bestThresshold(y_train,train_preds):
    ...
    return delta
delta = bestThresshold(y_val, val_preds)
