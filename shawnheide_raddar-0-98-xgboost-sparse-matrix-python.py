...
def xgb_gridsearch(X, y, params, xgbc = None, n_estimators=100):

    if xgbc == None:

        xgbc = xgb.XGBClassifier(n_estimators=n_estimators, silent=True, 

                                      objective='binary:logistic', nthread=1, seed=23)

    

    xgbc_gs = GridSearchCV(estimator = xgbc, param_grid = params, scoring='roc_auc', n_jobs=-1, cv=3, verbose=1)

    xgbc_gs.fit(X, y)

    print(xgbc_gs.best_estimator_)

    print(xgbc_gs.best_params_)

    

    return xgbr.best_estimator_
...
xgb_gridsearch(X_sparse, y, params)
